...
## Descripción
Diseña posts profesionales para redes sociales en Figma con la línea gráfica de cualquier marca. Extrae el ADN visual de la marca (colores, tipografía, tono, texturas) desde su sitio web y logos, y genera plantillas editables con todas las capas separadas directamente en Figma. Produce variaciones visuales por tipo de contenido sin salirse de la identidad aprobada.

## Triggers
- 'crea los posts en figma', 'diseña el contenido en figma', 'arma la parrilla en figma'
- 'genera los diseños de redes', 'hazme las plantillas en figma'
...
4. **Paleta de colores** → CSS custom properties, backgrounds, gradientes
5. **Tipografía** → font-family declarations
6. **Tono de comunicación** → Análisis del copy del sitio
7. **Estilo visual** → Dark/light theme, texturas, tipo de iconografía

**Se confirma con el usuario:**
8. **Formato de posts** → Dimensiones (1080×1080, 1080×1440, 1080×1920)
9. **Redes objetivo** → Instagram, Facebook, LinkedIn, TikTok
10. **¿Tiene Brand Kit en Canva?** → Para subir assets también ahí
...
6. Presentar al usuario para aprobación antes de diseñar
```
...
## 9. Aplicación por formato (HTML, DOCX, PPTX, Canva, Figma, matplotlib)
```

---

## Fase 2: Setup en Figma

### Pre-requisitos
- Figma MCP conectado (verificar con Figma:whoami)
- Archivo de Figma creado o proporcionado por el usuario
- Logos procesados (transparencia verificada con ImageMagick)
...
## Fase 3.5: Preview en HTML antes de Figma (OBLIGATORIO)

Antes de tocar Figma, Claude genera los diseños como HTML renderizado para aprobación del usuario. Esto ahorra llamadas MCP y evita iterar en Figma.

### Proceso
```
1. Claude genera un archivo HTML con TODOS los posts de la parrilla
   - Cada post a escala real (1080×[altura])
   - Con las variaciones visuales por tipo
   - Con logos embebidos en base64 (procesados con ImageMagick para transparencia)
   - Con texturas del isotipo como CSS background-image
   - Con gradientes, líneas decorativas, esquinas — todo fiel al branding
   
2. El usuario revisa los posts en su navegador
   - Valida textos, composición, colores, jerarquía
   - Pide ajustes (cambiar texto, mover elementos, corregir colores)
   
3. Claude itera en HTML hasta aprobación
   - Los cambios en HTML son instantáneos (sed, str_replace)
   - Sin consumir llamadas de Figma MCP
   
4. Una vez aprobado, Claude replica EXACTAMENTE el diseño en Figma
   - Ya sabe las posiciones, colores, textos y variaciones correctas
   - Ejecuta en mínimas llamadas use_figma (agrupar 3-5 posts por llamada)
   - Sin necesidad de screenshots de verificación (el usuario ya aprobó en HTML)
```

### Estructura del HTML de preview
```html
<!-- Cada post como un div escalado -->
<div class="post" style="width:1080px; height:[altura]px; transform:scale(0.45);">
  <!-- Fondo gradiente -->
  <!-- Textura isotipo (CSS pattern con base64) -->
  <!-- Isotipo fantasma decorativo -->
  <!-- Línea de acento top -->
  <!-- Esquinas decorativas -->
  <!-- Logo (base64 embebido) -->
  <!-- Tag pill -->
  <!-- Título con palabras en accent -->
  <!-- Divider -->
  <!-- Contenido (párrafo / pasos / stat) -->
  <!-- CTA -->
  <!-- Footer -->
</div>
```

### Generación de PNGs finales (opcional)
Si el usuario quiere PNGs listos para publicar sin pasar por Figma:
```bash
# Renderizar HTML a PNG con Playwright
pip install playwright --break-system-packages
playwright install chromium
python3 render_posts.py  # Script que captura cada .post como PNG individual
```

### Ventajas de este paso
- **Iteración rápida:** Cambios de texto/color en HTML son instantáneos
- **Cero consumo de Figma MCP:** Todo el diseño y aprobación ocurre aquí
- **Consistencia garantizada:** Lo que se aprueba en HTML es lo que va a Figma
- **PNGs de emergencia:** Si se acaba el límite de Figma, los PNGs están listos

---

## Fase 4: Construcción de cada post en Figma

### Código Figma — Estructura de capas (orden de creación)

Para cada post, Claude ejecuta `Figma:use_figma` con código que crea estas capas en orden:
...
├── 5. Logo (Frame placeholder O imagen real si está disponible)
...
│   ├── Pasos numerados (para educativo)
...
### Optimización de llamadas MCP (CRÍTICO para plan Starter)
El plan Starter de Figma MCP tiene límite de llamadas. Para optimizar:
...
3. **Evitar screenshots innecesarios:** Solo verificar visualmente al final, no después de cada post
...
4. Generar el código Figma para TODOS los posts
5. Ejecutar en lotes de ~5 posts por llamada Figma:use_figma
6. Cada post = 1 página en el archivo Figma
7. Nombrar páginas: "Post [N] — [Tipo] — [Tema corto]"
8. Verificar con screenshot al final
```
...
1. **Archivo Figma** con todas las páginas/posts editables
2. **branding-[cliente].md** — Brand Kit documentado
3. **Kit de elementos PNG** (ZIP) — fondos, logos, isotipos, texturas, líneas, esquinas — todo con transparencia para uso independiente
...
- Preguntar siempre: "¿hay algún color que NO deba usar?" (ej: verde de WhatsApp)
...
- Inter y Rajdhani son fallbacks seguros

### Sobre el logo
- Siempre procesar con ImageMagick para transparencia antes de subir
...
- Plan Starter: ~25 llamadas antes de rate limit
- Plan Professional: significativamente más
- Agrupar todo en mínimas llamadas `use_figma`
- No hacer screenshots de verificación después de cada post individual
- El usuario puede verificar directamente en Figma abriendo el archivo
